...
{
    using is_avalanching = void;
    auto operator()(std::pair<State, Belief> const &x) const noexcept -> uint64_t
    {
...
MetaNode::MetaNode(const State _state, const Belief _belief, MetaGraph *_meta):state(_state),belief(_belief), meta(_meta)
{
...
void MetaNode::expand()
{
    ...
    if(state == State{std::vector<ui>{1,1,0,0}}){
        std::cout << "let's see.." << std::endl;
    }

    expanded = true;
    // find the greedy actions corresponding to current belief and if we are an M-state
    auto &optimal_rewards = meta->optimal[state].expeced_gains;
    auto m_result = check_m_state(gains, optimal_rewards);

    // Add a child for pure termination
    add_child(m_result.indices[0], belief, true);
    ...
    if (!m_result.is_m_state)
    {
        ...
        {
            // if we already looked at an arm with identical wins/losses, we don't need to expand
    ...
    // expand all meta child nodes of this one, both for terminal actions and for computations
...
void MetaNode::add_child(ui arm, const Belief &new_belief, bool terminal)
{
    ...
    auto [states, edges] = subgraph(winning_child, new_belief.first, new_belief.second, meta->max_depth);
    Belief winning_belief{states, edges};

    auto [winning_it, inserted_win] = meta->nodes.try_emplace(winning_belief, MetaNode(winning_child, winning_belief, meta));

    
    auto losing_child = this->state;
    losing_child[2*arm+1] += 1;
    auto [states_losing, edges_losing] = subgraph(losing_child, new_belief.first, new_belief.second, meta->max_depth);
    Belief losing_belief{states_losing, edges_losing};
    auto [losing_it, inserted_lose] = meta->nodes.try_emplace(losing_belief, MetaNode(losing_child, losing_belief, meta));
    if (terminal)
    {
        terminal_children.try_emplace(arm, std::array<MetaNode *, 2>{&(winning_it->second), &(losing_it->second)});

    }
    else
    {
        computational_children.try_emplace(arm, std::array<MetaNode *, 2>{&(winning_it->second), &(losing_it->second)});
    }
}
...
void MetaNode::computational_expansion(ui terminal_action, ui candidate)
{
    ...
    std::cout << "Doing computational expansion at state " << this->state << ". Greedy action is " << terminal_action << ", we want to change our mind to " << candidate << "\n";
    ...
    {
        current_expansion.push_back(ExpansionCandidate{belief, get_candidates(candidate)});
    }
    // If not, we need to  check if we change our mind on the first try. Then we don't need to do the loop.
    else
    {
        ...
        auto new_gains = eval_basegraph(state, new_belief.first, new_belief.second, meta->max_depth);
        if (new_gains[candidate] > new_gains[terminal_action] + 1e-7)
        {
            // We changed our mind. Add metanodes for the winning and losing child and insert them to the children of this node.
    ...
    while (!current_expansion.empty())
    {
        ...
        {
            // Don't need to check beliefs twice. This is a necessary check, at least for performance.
            ...
            for (auto candidate_expansion : current_candidates)
            {
                ...
                // Check if the current candidate state is an M-state or not. If so, only expand in optimal directions.
                auto new_gains = eval_basegraph(candidate_expansion, current_belief.first, current_belief.second, meta->max_depth);
                auto &optimal_rewards = meta->optimal[candidate_expansion].expeced_gains;
                auto m_result = check_m_state(new_gains, optimal_rewards);
                auto &range = m_result.is_m_state ? m_result.indices : arms;
                // Expand the candidate along the chosen arm. Then, if we changed our mind, we found a mindchanger.
                for (auto arm : range)
                {
                    ...
                    {
                        // check if the new belief contains a minimal mindchanger. Then it is not minimal and is excluded from further consideration
                    ...
                    auto candidate_gains = eval_basegraph(state, new_belief.first, new_belief.second, meta->max_depth);
                    if (candidate_gains[terminal_action]+1e-7 < candidate_gains[candidate])
                    { // we changed our mind
                    ...
                    {
                        auto new_candidate = ExpansionCandidate{new_belief, current_candidates};
...
set<State, StateHash> MetaNode::get_candidates( ui candidate)
{
    ...
    for (auto node : belief.first)
    {
        ...
        auto new_gains = eval_basegraph(node, belief.first, belief.second, meta->max_depth);
